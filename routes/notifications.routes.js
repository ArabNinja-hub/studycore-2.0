...
router.post('/:id/read', (req, res) => {
  ...
  try {
    ...
    `).run(`ar-${uuidv4()}`, req.user.id, announcement.id, now);
  } catch (err) {
...
// Marks all published announcements as read for the current user.
router.post('/read-all', (req, res) => {
  ...
  for (const item of unreadRows) {
    try {
      insertStmt.run(`ar-${uuidv4()}`, req.user.id, item.id, now);
    } catch {
      // ignore
    }
  }
