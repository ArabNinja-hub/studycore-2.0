...
// A connected Google Drive vault (lib/google-drive-vault.js) is an equally
// durable, non-ephemeral place for DOCUMENT bytes to live, so it counts as a
// second acceptable production backend alongside R2. It does NOT cover video
// (which always goes to Bunny Stream) or the small app-managed assets
// (avatars, quiz images) that middleware/upload.js deliberately keeps off
// the vault - those still need R2 in production regardless of Drive. This
// check is intentionally best-effort/lazy (require() is deferred to avoid a
// require cycle with lib/storage.js, which this module is loaded from) and
// only softens the guard; it never that upgrades a real misconfiguration
// into a false "it's fine".
function driveVaultConfigured() {
  try {
...
if (!r2Configured) {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!driveVaultConfigured()) {
      ...
      // on every deploy. Fail startup loudly instead - unless a Google Drive
      // vault is already connected and durably storing documents instead.
      throw new Error(
        'FATAL: Cloudflare R2 must be configured in production (or connect a Google Drive vault ' +
        `in Admin > Integrations). Missing: ${missing.join(', ')}`
      );
    }
    console.log('='.repeat(60));
    console.log('StudyCore: Cloudflare R2 is not configured, but a Google Drive vault is connected.');
    console.log('Documents will be stored in the connected Google Drive account.');
    console.log('Avatars, quiz images and video thumbnails still require R2 - set it up when convenient.');
    console.log('='.repeat(60));
  } else {
    console.log('='.repeat(60));
