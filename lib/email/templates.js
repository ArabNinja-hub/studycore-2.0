...
// ---------------------------------------------------------------------------
// Single-active-device: new device login verification.
// Sent when someone signs into a STUDENT account that already has an active
// session on another device. Contains the 6-digit code AND the one-time magic
// link; either proof completes the device switch. Never includes passwords or
// session material - only the short-lived proofs and coarse device labels.
// ---------------------------------------------------------------------------
function deviceLoginVerification({ name, code, verifyUrl, newDeviceLabel, currentDeviceLabel, expiresInMinutes = 10 }) {
  const who = escapeHtml(firstName(name));
  const minutes = Number(expiresInMinutes) || 10;
  const safeCode = escapeHtml(String(code || '').replace(/[^\d]/g, '').slice(0, 6));
  const newDevice = escapeHtml(newDeviceLabel || 'a new device');
  const currentDevice = escapeHtml(currentDeviceLabel || 'another device');
  const link = verifyUrl || siteLink('/device-verify.html');

  const subject = 'StudyCore: New device login verification';
  const html = layout({
    preheader: 'Someone is attempting to sign into your StudyCore account from another device.',
    heading: `Verify your new device, ${who}`,
    ctaLabel: 'Verify this device',
    ctaHref: link,
    bodyHtml: [
      p(`Someone - hopefully you - just signed into your StudyCore account from <strong>${newDevice}</strong>. Your account is currently active on <strong>${currentDevice}</strong>, and for your security StudyCore keeps you signed in on only one device at a time.`),
      p('Enter this verification code on the new device to move your account to it. You can also tap the button below - either method verifies the switch.'),
      `
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:4px 0 18px 0;">
                <tr>
                  <td align="center" bgcolor="#eef9f7" style="background-color:#eef9f7;border:1px solid #d3eae6;border-radius:10px;padding:22px 16px;font-family:'Courier New',Courier,monospace;font-size:34px;letter-spacing:10px;font-weight:bold;color:${NAVY_900};text-align:center;">${safeCode}</td>
                </tr>
              </table>`,
      p(`This code and link <strong>expire in ${minutes} minutes</strong> and can each be used only once. Until you verify, your current device stays signed in and the new device cannot continue.`),
      p('If this was not you, you can safely ignore this email - nothing changes. Consider changing your password from your dashboard if you did not attempt to sign in.')
    ].join('\n'),
    footerNote: 'You are receiving this email because a login to your StudyCore account was attempted from another device.'
  });

  const text = [
    `Hi ${firstName(name)},`,
    '',
    `Someone - hopefully you - just signed into your StudyCore account from ${newDeviceLabel || 'a new device'}.`,
    `Your account is currently active on ${currentDeviceLabel || 'another device'}, and StudyCore keeps`,
    'each student signed in on only one device at a time.',
    '',
    `Your verification code is: ${safeCode}`,
    '',
    'Enter it on the new device, or open this secure link instead:',
    link,
    '',
    `The code and link expire in ${minutes} minutes and can each be used only once.`,
    'Until you verify, your current device stays signed in.',
    '',
    'If this was not you, ignore this email - nothing changes. Consider changing',
    'your password if you did not attempt to sign in.',
    '',
    'Stay curious and winning',
    'The StudyCore Team',
    '',
