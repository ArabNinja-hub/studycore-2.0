...
// submitted credential back inside an error message. That text is surfaced to
// the admin UI and written to email_log, so it is scrubbed here - the single
// chokepoint every error passes through - before it can escape this module.
//
// Two independent rules, so neither alone has to be perfect:
//   1. redact anything shaped like a Resend key / bearer token;
//   2. redact the live RESEND_API_KEY verbatim, in case its shape ever
//      changes and rule 1 stops matching it.
const KEY_LIKE_PATTERNS = [
  /\bre_[A-Za-z0-9_-]{6,}/g,          // Resend API key
  /\bBearer\s+[A-Za-z0-9._-]{8,}/gi,  // Authorization header value
  /\bsk_[A-Za-z0-9_-]{6,}/g           // generic secret-key convention
];

function redactSecrets(text) {
  let out = String(text == null ? '' : text);
  for (const pattern of KEY_LIKE_PATTERNS) out = out.replace(pattern, '[redacted]');
  const liveKey = String(process.env.RESEND_API_KEY || '').trim();
  if (liveKey.length >= 8) out = out.split(liveKey).join('[redacted]');
  return out;
}
...
function describeError(error) {
  ...
  const raw = typeof error === 'string'
    ? error
    : `${error.name ? `${error.name}: ` : ''}${error.message || 'Unknown email error'}`;
