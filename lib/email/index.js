...
/**
 * ACTIVE. New device login verification - sent when a STUDENT account with
 * an active session on one device signs in from another. Contains a 6-digit
 * code and a one-time magic link; either proof completes the device switch.
 * Trigger: POST /api/auth/login (challenge path) and
 * POST /api/auth/device-verify/resend (routes/auth.routes.js).
 *
 * @param {object} args  { userId, name, email, challengeId, attempt, code,
 *                         verifyUrl, newDeviceLabel, currentDeviceLabel,
 *                         expiresInMinutes }
 * The code and link are delivery material only - they are never written to
 * email_log, never logged, and only SHA-256 hashes persist in the database.
 */
async function sendDeviceLoginVerificationEmail({ userId, name, email, challengeId, attempt, code, verifyUrl, newDeviceLabel, currentDeviceLabel, expiresInMinutes }) {
  return dispatch({
    kind: 'device_login_verification',
    // One email per send attempt of one challenge: resending the SAME
    // challenge is a fresh logical event, but a retried HTTP request cannot
    // ever double-send the same attempt.
    dedupeKey: `devlogin:${challengeId}:${Number(attempt) || 1}`,
    to: email,
    userId,
    message: templates.deviceLoginVerification({
      name,
      code,
      verifyUrl,
      newDeviceLabel,
...
module.exports = {
  ...
  sendDeviceLoginVerificationEmail,
