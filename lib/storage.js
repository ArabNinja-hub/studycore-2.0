...
// Multipart tuning for the R2 leg of an upload. Peak extra memory is
// roughly partSize * queueSize (~32MB here), which is well within a small
// dyno while being far faster than the SDK's 5MB / 4-part default.
const R2_PART_SIZE = 8 * 1024 * 1024;
const R2_QUEUE_SIZE = 4;
...
// Most callers that deal with app-managed assets (avatars and quiz images)
// want whichever backend is configured *now*, which remains the default.
// Resource documents are different: their database row records where that
// particular object was written. Honour an explicit provider so enabling R2
// later does not make an older local-disk document appear to vanish, and a
// later storage change cannot redirect an old R2 key to the wrong place.
...
function storageUnavailable(provider) {
  const err = new Error(`${provider === 'r2' ? 'Cloudflare R2' : 'Storage'} is not configured.`);
  err.code = 'StorageNotConfigured';
...
async function putObject({ key, body, contentType }) {
  ...
  if (r2Configured) {
    const upload = new Upload({
      ...
      // The default 5MB part size turns a 200MB lecture video into 40
      // sequential-ish round trips to R2, and each one adds latency on top
      // of the (already slow) client uplink. Larger parts with real
      // concurrency keep the R2 leg from becoming the bottleneck while
      // capping in-process buffering at partSize * queueSize.
...
async function headObject(key, provider) {
  ...
  if (backend === 'r2') {
    if (!r2Configured) throw storageUnavailable('r2');
    try {
...
async function readBytes(key, start, end, provider) {
  const obj = await getObject(key, { start, end }, provider);
...
// Temporary video-upload chunks must never enter R2. These local-only helpers
// are intentionally used only by the resumable Bunny ingest path; completed
// videos are streamed to Bunny and the chunks are removed immediately.
async function putLocalObject({ key, body }) {
  const k = safeKey(key);
  const dest = localPath(k);
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  if (body && typeof body.pipe === 'function') await pipeline(body, fs.createWriteStream(dest));
  else if (Buffer.isBuffer(body) || typeof body === 'string') await fs.promises.writeFile(dest, body);
  else throw new Error('Unsupported upload body');
  return { backend: 'local', key: k };
}
...
async function getLocalObject(key) {
  ...
  const filePath = localPath(k);
  let st;
  try { st = await fs.promises.stat(filePath); }
  catch (err) { if (err.code === 'ENOENT') throw notFound(); throw err; }
  return { body: fs.createReadStream(filePath), contentLength: st.size, contentType: null, contentRange: null };
}

async function deleteLocalObject(key) {
  if (!key) return;
  try { await fs.promises.unlink(localPath(key)); }
