...
function iframeUrl(uid, opts) {
  ...
  // View-only playback. These are Cloudflare Stream player options, applied
  // on Cloudflare's side inside the iframe — StudyCore's own page scripts
  // cannot reach into a cross-origin frame to hide a control, so the only
  // way to remove them is to ask for a player that never renders them.
  //
  //   defaultTextTrack is untouched, and NOTHING here limits playback:
  //   quality selection, seeking, volume, speed and fullscreen all stay.
  //
  // `preload=metadata` keeps mobile data use low on Zambian networks while
  // still letting the player show a duration before the student presses play.
  params.set('preload', 'metadata');
  const qs = params.toString();
  return `https://${sub}.cloudflarestream.com/${encodeURIComponent(uid)}/iframe${qs ? `?${qs}` : ''}`;
}

// Cloudflare Stream can be told, per video, to refuse the MP4 "downloads"
// endpoint and to require signed playback tokens. Neither is something a
// browser can be trusted to enforce, which is precisely why they belong on
// the Cloudflare side. Exposed here so the admin upload path can apply them
// (see routes/admin.routes.js) rather than each caller re-deriving the flags.
function signedPlaybackRequired() {
  return String(process.env.CF_STREAM_SIGNED || '').toLowerCase() === 'true';
}
