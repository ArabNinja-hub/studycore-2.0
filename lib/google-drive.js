...
// and written into StudyCore's own document storage — the connected Google
// Drive VAULT account when one is configured (lib/google-drive-vault.js), or
// R2/local disk otherwise (see lib/document-storage.js). From that moment
// the resource is an ordinary StudyCore document - served by the protected,
// session-gated /stream endpoint like any other upload. The Picker's OWN
// access token (the uploader's personal Drive authorization) is never
// reused beyond this one fetch, and no Drive URL, id or sharing state from
// the picked file ever reaches a student's browser.
//
// SCOPE / PERMISSIONS
//
...
async function importToStorage({ fileId, accessToken, fileName, mimeType } = {}) {
  ...
  try {
    ...
    if (written) await documentStorage.deleteObject(written.key, written.backend).catch(() => {});
