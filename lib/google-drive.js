...
// WHY THIS EXISTS
//
// Picking a file from Google Drive must not leave students dependent on that
// file's own Drive sharing settings. When StudyCore only stored a Drive file
// id and embedded / proxied that private Drive document, some students who were
// not personally shared on the uploader's file were shown Google's
// "You need access / Request access" page instead of their StudyCore material.
//
// The safe behaviour is the older, reliable one: at publish time the bytes are
// pulled out of Drive once, using the short-lived access token the Picker just
// granted, validated, and written into StudyCore's normal document storage
// (the connected StudyCore Drive vault when configured, otherwise R2/local via
// lib/document-storage.js). From that moment the resource is an ordinary
// StudyCore document served by the protected /api/resources/:id/stream endpoint
// like any other upload. Students never receive the Picker token, never receive
// the source Drive URL, and never have to request access in Google Drive.
//
// SCOPE / PERMISSIONS
//
// The Picker grants per-file access under the non-restricted `drive.file`
// scope: a file the uploader explicitly selects becomes readable by this
// client. That is exactly (and only) what is used here — StudyCore never asks
// for broad Drive read access and never enumerates the uploader's Drive.
//
// SECURITY
//
//   * The OAuth access token is used for this single ingest and then dropped.
//     It is never persisted, never logged, and never returned in any response.
...
//   * Native Google Workspace files (Docs/Sheets/Slides) have no downloadable
//     binary content, so they are exported to PDF on the way in — which also
//     keeps them readable inside the StudyCore PDF viewer.
// =============================================================================
...
async function driveRequest(url, accessToken, { timeoutMs = 60000 } = {}) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw driveError('Google Drive refused to release this file. Re-select it with "Select from Google Drive" so StudyCore can import it.', 403);
    }
    if (response.status === 404) {
      throw driveError('That Google Drive file could not be found. It may have been moved, deleted, or not granted to this Picker session - re-select it from the Picker.', 404);
    }
...
/**
 * Copy one Google Drive file into StudyCore's document storage.
 *
...
async function importToStorage({ fileId, accessToken, fileName, mimeType } = {}) {
  ...
  // Videos are published to Bunny Stream, never copied into document storage.
  ...
  const response = await driveRequest(downloadUrl, accessToken, {
    // Large PDFs over a slow egress path need more than the metadata timeout.
    // Matches the generosity of the ordinary upload path.
  ...
  // Stream Drive -> hash/measure -> document storage. The file is never
  // buffered whole in this process, exactly like the multipart upload path.
  ...
  let written;
  try {
    written = await documentStorage.putObject({
      key,
    ...
    // Local/R2 streams may have created a partial object before an error such
    // as the max-size guard fired. Delete by the generated key even when the
    // storage backend rejected before returning its usual { key, backend }.
    await documentStorage.deleteObject(written?.key || key, written?.backend).catch(() => {});
  ...
  if (size === 0) {
    await documentStorage.deleteObject(written.key, written.backend).catch(() => {});
