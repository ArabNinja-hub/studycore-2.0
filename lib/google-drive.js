...
// registerFile() therefore reads the picked file's metadata AS THE SERVER, via
// the very same call the student stream performs. If that fails, the publish
// is refused with an actionable message instead of publishing a resource that
// would later show every student "Document unavailable" — the exact symptom
// of publishing a Drive reference the backend cannot actually read.
//
// SCOPE / PERMISSIONS
//
...
// served as the PDF Drive exports them to (lib/drive-documents.js performs
// the export on demand, short-term cached in memory). At registration time
// only the file type is checked — no export or download happens at publish.
// =============================================================================
