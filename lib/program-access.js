...
function programCanSeeResource(user, row) {
  ...
  // Course membership is an additional restriction, even for "All Programs".
  // Check it before the targeting shortcut so direct links and SQL listings
  // enforce the same Student → Program → Course → Resource boundary.
  if (row.course_id && !programIncludesCourse(programCode, row.course_id)) return false;
