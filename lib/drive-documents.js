...
function keepFailedResponse(previous, next) {
  ...
  const nextDenied = next.status === 401 || next.status === 403;
  if (previousDenied && !nextDenied) return previous;
  return next;
}
...
async function getMetadata(fileId, { fresh = false } = {}) {
  ...
  if (!res.ok) {
    if (res.status === 404) {
      throw notFound('This document could not be found in Google Drive. It may have been deleted or moved out of the library.');
    }
    if (res.status === 403 || res.status === 401) {
      ...
      throw accessError('StudyCore\'s Google Drive connection can no longer read this document.');
    }
