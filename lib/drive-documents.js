// =============================================================================
// StudyCore — READ documents straight out of Google Drive
// -----------------------------------------------------------------------------
// Google Drive IS the document storage. This module is the read side of that:
// it exposes the same tiny object-storage interface as lib/storage.js (R2 /
// local disk) and lib/google-drive-vault.js —
//
...
// dispatch `storage_provider = 'google_drive'` here and the student stream
// route (routes/resources.routes.js) serves a Drive-hosted document through
// exactly the same code path — and the same access control — as any other
// document:
//
//     Google Drive -> StudyCore backend -> StudyCore document viewer
//
// Nothing is ever copied into StudyCore storage. There is no migration, no
// duplicate and no "being moved" state: a document published from Drive in
// 2024 and one picked from Drive a minute ago are read the same way.
//
...
//      has already link-shared it. StudyCore never changes a file's sharing
//      to make this work.
//
