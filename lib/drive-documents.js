...
// Metadata access is not enough to prove that the viewer can open a file.
// Drive can allow files.get(fields=...) while refusing files.get(alt=media)
// with 403 insufficientFilePermissions. That used to let a broken reference
// publish successfully and only fail later on the student's first read. Probe
// the actual media/export path at publish time so a resource can never enter
// the library in that state.
async function verifyReadable(key, suppliedMeta = null) {
  const id = String(key || '').trim();
  const meta = suppliedMeta || await getMetadata(id, { fresh: true });
  if (meta.isWorkspace) {
    // Workspace files have no alt=media bytes. exportWorkspaceFile() is the
    // exact path used by student reads and also validates the export limit.
    const entry = await exportWorkspaceFile(id, meta);
    if (!entry || !entry.buffer || !entry.buffer.length) {
      throw driveError('Google Drive returned an empty document export.', 502);
    }
    return meta;
  }

  // Ask for one byte only. Drive normally answers 206; getObject also handles
  // Google abuse acknowledgements and credential recovery exactly as the
  // viewer path does. We consume the returned stream so the fetch connection
  // is released before publishing completes.
  const probe = await getObject(id, { start: 0, end: 0 });
  let received = 0;
  for await (const chunk of probe.body) {
    received += Buffer.byteLength(chunk);
    if (received > 0) break;
  }
  if (probe.body && typeof probe.body.destroy === 'function') probe.body.destroy();
  if (!received) throw driveError('Google Drive returned an empty document.', 502);
  return meta;
}
