...
//
// Google Drive is StudyCore's document SOURCE LIBRARY, not its storage.
// An admin organises their notes, tutorials, past papers and lab reports in
// their own Drive and uses "Select from Google Drive" (the Drive Picker) to
// bring a file in. lib/google-drive.js copies the picked file into StudyCore's
// own storage once, and the student viewer serves it from there.
//
// This module is the OPTIONAL server-side Drive connection behind that: one
// Main Admin connects ONE Google account, and StudyCore stores an encrypted
// refresh token so the server can talk to Drive on its own behalf. It is used
// for:
//
//   * reading legacy rows whose bytes were never imported and still live in
//     Drive (see lib/drive-documents.js), and
//   * reading rows written by an older build that used this account as a
//     storage vault (storage_provider = 'google_drive_vault').
//
...
// OPTIONAL
//
// Nothing here is required. With no account connected, `isConfigured()` is
// false, direct uploads and Picker imports both go to R2/local exactly as
// normal, and the Picker still works (it authorises per file in the admin's
// own browser).
// =============================================================================
