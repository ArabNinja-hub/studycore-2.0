...
//
// StudyCore imports files selected from an uploader's Google Drive so students
// never depend on that uploader's private Drive sharing list (the source of the
// old Google "Request access" wall). This module is the optional storage
// backend those imported bytes are written TO: one connected StudyCore Drive
// vault account. If no vault is connected, imports fall back to R2/local.
//
// The vault can also store ordinary multipart/resumable document uploads. It
// keeps the same access guarantee as every other StudyCore storage backend by
// NEVER exposing a Drive URL, a Drive file id, or Drive sharing to a student's
// browser:
//
...
//
// Net effect: the connected StudyCore vault can be the storage unit, but
// "opening a document" is still entirely gated by StudyCore's own
// login/subscription rules, because the browser never talks to Drive directly.
// When the source was selected from a user's Drive, the bytes are imported into
// this vault (or R2/local) first, so source-file sharing cannot affect students.
//
// FALLBACK
//
