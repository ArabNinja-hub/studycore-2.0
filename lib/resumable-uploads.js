...
const bunnyStream = require('./stream');
...
function isVideoSession(session) {
  ...
  return session.storage_provider === 'bunny' ||
    VIDEO_EXTENSIONS.has(path.extname(String(session.file_name || '')).toLowerCase()) ||
    String(session.mime_type || '').toLowerCase().startsWith('video/');
}
...
function createSession({ userId, fileName, fileSize, mimeType, chunkSize }) {
  ...
  db.prepare(`
    INSERT INTO upload_sessions (
      ...
      storage_key, status, created_at, updated_at, expires_at, storage_provider, stream_uid
    ) VALUES (
      @id, @user_id, @file_name, @file_size, @mime_type, @chunk_size, @total_chunks,
      NULL, 'open', @created_at, @created_at, @expires_at, @storage_provider, NULL
    )
  `).run({
    id,
    ...
    storage_provider: (VIDEO_EXTENSIONS.has(path.extname(String(fileName || '')).toLowerCase()) ||
      String(mimeType || '').toLowerCase().startsWith('video/')) ? 'bunny' : documentStorage.backendName()
  });
...
async function putPart(session, partNumber, body, declaredLength) {
  ...
  // In-flight chunks are always temporary scratch objects — they are
  // reassembled and deleted the moment the session completes — so they
  // stay on the pre-existing R2/local backend even when the Google Drive
  // vault is the final destination for the assembled document. This avoids
  // creating (and rate-limiting against) dozens of tiny Drive files per
  // upload; only the one finished object below is written to Drive.
  if (isVideoSession(session)) {
    // Temporary video chunks stay on Render/local disk and never enter R2.
    await storage.putLocalObject({ key, body: meter });
  } else {
  ...
  if (written !== expected) {
    await (isVideoSession(session) ? storage.deleteLocalObject(key) : storage.deleteObject(key)).catch(() => {});
...
function assembledStream(session) {
  ...
  async function* generate() {
    for (let i = 0; i < total; i += 1) {
      const obj = isVideoSession(session)
        ? await storage.getLocalObject(partKey(session.id, i))
        : await storage.getObject(partKey(session.id, i));
      const body = obj.body;
...
async function finalizeSession(session, { validateHead, title } = {}) {
  ...
  if (isVideoSession(session) && !bunnyStream.isConfigured()) {
  ...
  let video = null;
  let writtenKey = key;
  let writtenBackend = null;
  if (isVideoSession(session)) {
    try {
      video = await bunnyStream.uploadFromStream(metered, {
        name: String(title || '').trim() || session.file_name,
        fileName: session.file_name,
        contentType: 'application/octet-stream',
        contentLength: session.file_size
      });
    } catch (cause) {
      const err = new Error('Video upload failed. Bunny Stream did not accept the file; no video was published. Please try again.');
      err.statusCode = 502;
      err.userSafe = true;
      err.cause = cause;
      throw err;
    ...
    writtenBackend = written.backend;
  }

  // Same magic-byte guarantee as multipart uploads. If a video was already
  // accepted by Bunny, remove it before rejecting the disguised file.
  if (typeof validateHead === 'function' && !validateHead(head.subarray(0, headLen), ext)) {
    if (video && video.uid) await bunnyStream.deleteVideo(video.uid).catch(() => {});
    else await documentStorage.deleteObject(writtenKey, writtenBackend).catch(() => {});
  ...
  try {
    db.prepare(`
      ...
      SET status = 'complete', storage_key = ?, stream_uid = ?, storage_provider = ?, updated_at = ?
      WHERE id = ?
    `).run(video ? null : writtenKey, video ? video.uid : null, video ? 'bunny' : writtenBackend, nowIso(), session.id);
  } catch (err) {
    if (video && video.uid) await bunnyStream.deleteVideo(video.uid).catch(() => {});
    else await documentStorage.deleteObject(writtenKey, writtenBackend).catch(() => {});
    throw err;
  ...
  return {
    key: video ? null : writtenKey,
    size,
    contentHash: hash.digest('hex'),
    bucket: video ? 'bunny' : writtenBackend,
    streamUid: video ? video.uid : null,
    streamStatus: video ? (video.status || 'queued') : null,
    streamDuration: video ? (video.duration || null) : null,
...
async function deleteChunks(session) {
  ...
  for (const part of parts) {
    await (isVideoSession(session)
      ? storage.deleteLocalObject(partKey(session.id, part))
      : storage.deleteObject(partKey(session.id, part))).catch(() => {});
  }
...
async function discardSession(sessionId) {
  ...
  if (!session) return;
  await deleteChunks(session);
  if (session.status !== 'claimed' && session.stream_uid) {
    await bunnyStream.deleteVideo(session.stream_uid).catch(() => {});
  } else if (session.storage_key && session.status !== 'claimed') {
    await documentStorage.deleteObject(session.storage_key, session.storage_provider).catch(() => {});
  }
