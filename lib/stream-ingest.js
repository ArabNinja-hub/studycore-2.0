...
//   2. POST them to Bunny Stream, which encodes adaptive renditions.
...
// Offload one already-inserted video resource row to Bunny Stream.
...
async function offloadResourceToStream(resource) {
  ...
  try {
    ...
    console.warn('StudyCore Stream: upload to Bunny Stream failed:', err.message);
...
// (Bunny's basic upload is not streaming), so letting a batch of uploads
